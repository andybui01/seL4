...
static void lai_exec_mutate_ns(lai_nsnode_t *target, lai_variable_t *object) {
    switch (target->type) {
        case LAI_NAMESPACE_NAME:
            switch (target->object.type) {
                ...
                case LAI_PACKAGE: {
                    ...
                    word_t n = lai_exec_pkg_size(object);
    ...
    switch (dest->tag) {
        ...
        case LAI_DEBUG_NAME:
            if (laihost_handle_amldebug)
                ...
                switch (object->type) {
                    case LAI_INTEGER:
                        lai_debug("Debug(): integer(%lld)", object->integer);
                        break;
                    case LAI_STRING:
                        lai_debug("Debug(): string(\"%s\")", lai_exec_string_access(object));
                        break;
                    case LAI_BUFFER:
                        lai_debug("Debug(): buffer(%lX)", (word_t)lai_exec_buffer_access(object));
                        break;
                    default:
...
static void lai_write_buffer(lai_nsnode_t *handle, lai_variable_t *source) {
    ...
    word_t offset = handle->bf_offset;
    word_t size = handle->bf_size;
    uint8_t *data = handle->bf_buffer->content;

    word_t n = 0; // Number of bits that have been written.
    while (n < size) {
