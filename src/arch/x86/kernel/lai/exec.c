...
#include <lai/internal/util-list.h>
#include <lai/internal/util-macros.h>
...
static lai_api_error_t lai_exec_reduce_op(int opcode, lai_state_t *state,
    ...
    switch (opcode) {
            ...
            switch (operand0_convert.type) {
                ...
                case LAI_BUFFER: {
                    if (operand1_convert_temp.type == LAI_STRING) {
                        word_t strl = lai_exec_string_length(&operand1_convert_temp);
                    ...
                    word_t b0size = lai_exec_buffer_size(&operand0_convert);
                    word_t b1size = lai_exec_buffer_size(&operand1_convert);
                ...
                case LAI_STRING: {
                    switch (operand1_convert_temp.type) {
                        ...
                        case LAI_BUFFER: {
                            // word_t length = lai_exec_buffer_size(&operand1_convert_temp);
                            lai_api_error_t error = lai_create_string(&operand1_convert, 0);
                            if (error != LAI_ERROR_NONE) {
                                lai_warn("failed to allocate memory for AML string");
                                return error;
                    ...
                    word_t s0len = lai_exec_string_length(&operand0_convert);
                    word_t s1len = lai_exec_string_length(&operand1_convert);
        ...
        case MATCH_OP: {
            ...
            word_t start_index = start_index_var.integer;

            result.type = LAI_INTEGER;
            result.integer = ~((uint64_t)0); // OnesOp

            word_t package_size = lai_exec_pkg_size(&package);
            for (word_t i = start_index; i < package_size; i++) {
        ...
        case CONCATRES_OP: {
            ...
            word_t buf1_size = lai_exec_buffer_size(&buf1_var);
            const char *buf1 = lai_exec_buffer_access(&buf1_var);

            word_t buf2_size = lai_exec_buffer_size(&buf2_var);
            ...
            word_t result_size = (buf1_size - 2) + (buf2_size - 2)
            ...
            uint8_t check = 0;
            for (word_t i = 0; i < (result_size - 1); i++)
...
static word_t lai_parse_varint(word_t *out, uint8_t *code, int *pc, int limit) {
    if (*pc + 1 > limit)
        return 1;
    uint8_t sz = (code[*pc] >> 6) & 3;
    if (!sz) {
        *out = (word_t)(code[*pc] & 0x3F);
        (*pc)++;
        return 0;
    } else if (sz == 1) {
        if (*pc + 2 > limit)
            return 1;
        *out = (word_t)(code[*pc] & 0x0F) | (word_t)(code[*pc + 1] << 4);
        *pc += 2;
        return 0;
    } else if (sz == 2) {
        if (*pc + 3 > limit)
        ...
        *pc += 3;
        ...
        if (*pc + 4 > limit)
            return 1;
        *out = (word_t)(code[*pc] & 0x0F) | (word_t)(code[*pc + 1] << 4)
               | (word_t)(code[*pc + 2] << 12) | (word_t)(code[*pc + 3] << 20);
        *pc += 4;
        return 0;
    }
}
...
static lai_api_error_t lai_exec_parse(int parse_mode, lai_state_t *state) {
    ...
    switch (opcode) {
        ...
        case STRINGPREFIX: {
            int data_pc;
            word_t n = 0; // Length of null-terminated string.
            while (pc + n < (word_t)block->limit && method[pc + n])
                n++;
            if (pc + n == (word_t)block->limit)
        ...
        case BUFFER_OP: {
            int data_pc;
            word_t encoded_size; // Size of the buffer initializer.
        ...
        case VARPACKAGE_OP: {
            int data_pc;
            word_t encoded_size; // Size of the package initializer.
            if (lai_parse_varint(&encoded_size, method, &pc, limit))
                return LAI_ERROR_EXECUTION_FAILURE;
            data_pc = pc;
            pc = opcode_pc + 1 + encoded_size;
