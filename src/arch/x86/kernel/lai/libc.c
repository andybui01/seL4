...
void *lai_calloc(word_t count, word_t item_size) {
    word_t size = count * item_size;
...
word_t lai_strlen(const char *s) {
    word_t len = 0;
    for (word_t i = 0; s[i]; i++)
        len++;
    return len;
}
...
void lai_snprintf(char *buf, word_t bufsize, const char *fmt, ...) {
    va_list list;
    va_start(list, fmt);
