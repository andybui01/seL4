...
static unsigned int lai_hash_string(const char *str, word_t n) {
...
lai_api_error_t lai_install_nsnode(lai_nsnode_t *node) {
    ...
    if (instance->ns_size == instance->ns_capacity) {
        word_t new_capacity = instance->ns_capacity * 2;
        if (!new_capacity)
            new_capacity = 128;
...
void lai_uninstall_nsnode(lai_nsnode_t *node) {
    ...
    for (word_t i = 0; i < instance->ns_size; i++) {
...
word_t lai_amlname_parse(struct lai_amlname *amln, const void *data) {
    amln->is_absolute = 0;
    amln->height = 0;
...
char *lai_stringify_amlname(const struct lai_amlname *in_amln) {
    ...
    word_t num_segs = (amln.end - amln.it) / 4;
    word_t max_length = 1 // Leading \ for absolute paths.
...
char *lai_stringify_node_path(lai_nsnode_t *node) {
    ...
    word_t length = num_segs * 5; // Leading dot (or \) and four chars per segment.
...
word_t lai_resolve_new_node(lai_nsnode_t *node, lai_nsnode_t *ctx_handle, void *data) {
    struct lai_amlname amln;
    word_t size = lai_amlname_parse(&amln, data);
...
lai_nsnode_t *lai_ns_child_iterate(struct lai_ns_child_iterator *iter) {
    while (iter->i < (word_t)iter->parent->children.elem_capacity) {
