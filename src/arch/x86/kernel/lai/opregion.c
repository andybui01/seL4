...
#include <lai/internal/opregion.h>

// this assumes little endian
static void lai_buffer_put_at(uint8_t *buffer, uint64_t value, word_t bit_offset, word_t num_bits) {
    word_t progress = 0;
    while (progress < num_bits) {
        word_t in_byte_offset = (bit_offset + progress) & 7;
        word_t access_size = LAI_MIN(num_bits - progress, 8 - in_byte_offset);
        word_t mask = (1 << access_size) - 1;
...
// this assumes little endian
static uint64_t lai_buffer_get_at(uint8_t *buffer, word_t bit_offset, word_t num_bits) {
    uint64_t value = 0;
    word_t progress = 0;
    while (progress < num_bits) {
        word_t in_byte_offset = (bit_offset + progress) & 7;
...
static word_t lai_calculate_access_width(lai_nsnode_t *field) {
...
static uint64_t lai_perform_read(lai_nsnode_t *opregion, word_t access_size, word_t offset) {
...
static void lai_perform_write(lai_nsnode_t *opregion, word_t access_size, word_t offset,
                              uint64_t value) {
    struct lai_instance *instance = lai_current_instance();
    if (opregion->op_override) {
        if (instance->trace & LAI_TRACE_IO)
...
static uint64_t lai_perform_indexfield_read(lai_nsnode_t *opregion, word_t access_size,
                                            word_t offset) {
    (void)(access_size);
...
static void lai_perform_indexfield_write(lai_nsnode_t *opregion, word_t access_size, word_t offset,
                                         uint64_t value) {
    (void)(access_size);
