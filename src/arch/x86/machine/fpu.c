...
void initFpuContext(tcb_t *tcb)
{
    ...
    memcpy(&tcb->tcbArch.tcbFpu.tcbBoundFpu->state[0], x86KSnullFpuState, CONFIG_XSAVE_SIZE);
}
...
BOOT_CODE bool_t Arch_initFpu(void)
{
    ...
    if (config_set(CONFIG_XSAVE)) {
        ...
        memzero(x86KSnullFpuState, CONFIG_XSAVE_SIZE);
        ...
        doSaveFpuState(x86KSnullFpuState);
